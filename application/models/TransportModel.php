...
class TransportModel extends CI_Model {
    ...
    //         $PENALTY = ($BILL_DT > $Transit_Time) ? '1500' : 0 ;
    ...
    //         $interval = $datetime1->diff($datetime2);
    //         $DETENTION= ($interval->format('%d')*24) + $interval->format('%H');
    //         $DETENTION = ($DETENTION > 24) ? '1500' : 0 ;
    ...
    //         // $config['upload_path']          = 'http://10.173.107.5:8080/freight/assets/';//file save path
    ...
    public function save_temp_trans($postData)
    {
        ...
        if($query->num_rows() > 0){
            ...
            if($postData["tid"]){
                $update = $this->db->where('ID', $postData['tid'])->update('TRANSINVOICE', array('SUBOTHCHRGS' => $subothercharge));
            }
