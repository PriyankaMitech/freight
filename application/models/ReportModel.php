...
class ReportModel extends CI_Model {
    ...
    public function getDetention() {
        $query = $this->db->query(
            ...
            from TRANSINVOICE TR join tbl_sales SA ON TR.SALES_ID=SA.ID group by TR.LR_NO ");
    ...
    public function getdieselCostwithfreight($whereCond) {
        ...
        $this->db->select(" SA.ID as ID, IF(TR.FREIGHT_T > 0,TR.FREIGHT_T, 0) 'FREIGHT_T',IF(SA.FRT_AFT_ST > 0,SA.FRT_AFT_ST, 0) 'FRT_AFT_ST',SUM(TR.BOX_QTY_T) as BOX_QTY_T,SA.NAME_SHIP_PARTY as NAME_SHIP_PARTY, TR.TOTAL_VAL as TOTAL_VAL,SA.LCN_SHIP_PARTY as LCN_SHIP_PARTY,SA.BILLING_DOC as BILLING_DOC, SA.REF_NO, TR.BILL_DT as BILL_DT, SUM(TR.BILL_QTY_T) as BILL_QTY_T,TR.DETENTION as DETENTION, TR.PENALTY as PENALTY,TR.OTHCHRGS as OTHCHRGS,  SA.SHIP_TO_PARTY as SHIP_TO_PARTY, SA.VENDOR as VENDOR,SA.NAME_SHIP_PARTY, SA.LCN_SHIP_PARTY, SA.VEH_NO, SA.COUNTRY_KEY, TR.LR_NO as LR_NO, SA.VEH_NAME as VEH_NAME, SA.INCOTERMS, TR.TRANS_NAME as TRANS_NAME, SA.STATE_NM_SOLD, CA.Rate as Rate, VM.VEHICLE_AVERAGE as VEHICLE_AVERAGE, VM.VEHICLE_LOAD_ABILITY as VEHICLE_LOAD_ABILITY ");
        $this->db->join('tbl_sales SA', 'TR.SALES_ID=SA.ID');
        //$this->db->join('tbl_kilometer KT', 'KT.destination=SA.LCN_SHIP_PARTY');
        $this->db->join('tbl_vehicle_master1 VM', 'VM.VEHICLE_NAME=SA.VEH_NAME');
        $this->db->join('tbl_contract CA', 'CA.Destination=SA.LCN_SHIP_PARTY and CA.Vehicle_Name=SA.VEH_NAME');
